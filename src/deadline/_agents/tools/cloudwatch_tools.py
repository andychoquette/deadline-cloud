...
def get_cloudwatch_client(
    farm_id: str = None, queue_id: str = None, config: Optional[ConfigParser] = None
):
    """
...
def get_cloudwatch_log_events(
    ...
    try:
        ...
        logger.info("Calling get_log_events with params:")
        ...
        try:
            while page_count < max_pages:
                ...
                logger.info(
                    f"Page {page_count}: Retrieved {len(events)} events (total so far: {len(all_events)})"
                )
            ...
            # Log at DEBUG level to avoid cluttering customer output
            logger.debug("=" * 80)
            logger.debug("CLOUDWATCH API ERROR")
            logger.debug("=" * 80)
            ...
            if "ResourceNotFoundException" in error_str:
                if "log stream does not exist" in error_str:
                    return (
                        f"⚠️ Log stream not found: `{log_stream_name}`\n\n"
                        f"**Possible reasons:**\n"
                        f"1. The log stream name from session details may be incorrect\n"
                        f"2. The session may not have generated any logs yet\n"
                        f"3. The session may have used a different log stream name format\n\n"
                        f"**Next steps:**\n"
                        f"- Verify you're using the EXACT log stream name from `get_deadline_session_details`\n"
                        f"- Check if the session actually ran (check session status)\n"
                        f"- Try listing log streams in the log group to see what's available\n\n"
                        f"Log Group: `{log_group_name}`\n"
                        f"Log Stream (attempted): `{log_stream_name}`"
                    )
                elif "log group does not exist" in error_str:
                    return (
                        f"⚠️ Log group not found: `{log_group_name}`\n\n"
                        f"**This means CloudWatch logging may not be configured for this queue.**\n\n"
                        f"**Next steps:**\n"
                        f"- Verify the queue has CloudWatch logging enabled\n"
                        f"- Check the queue configuration for log settings\n"
                    )

            if "AccessDeniedException" in error_str:
                return (
                    f"⚠️ Access denied to CloudWatch logs.\n\n"
                    f"**Issue**: The queue role does not have permission to access this log group.\n\n"
                    f"**Solution**: Ensure the queue role has `logs:GetLogEvents` permission for:\n"
            ...
            return f"⚠️ CloudWatch API error: {error_str}"
...
def get_worker_cloudwatch_logs(
    ...
    try:
        ...
        try:
            ...
            response = deadline_client.assume_fleet_role_for_read(farmId=farm_id, fleetId=fleet_id)
            ...
            logger.error(traceback.format_exc())
            return (
                f"❌ Failed to assume fleet role or create CloudWatch client: {str(client_error)}"
            )
        ...
        logger.info("Calling get_log_events for worker logs...")
        ...
        try:
            while page_count < max_pages:
                ...
                logger.info(
                    f"Page {page_count}: Retrieved {len(events)} events (total: {len(all_events)})"
                )
        ...
        result = (
            f"Found {len(all_events)} worker log event(s) in {log_group_name}/{log_stream_name}"
        )
        if start_time or end_time:
            result += " (filtered by time range)"
        result += ":\n\n"
